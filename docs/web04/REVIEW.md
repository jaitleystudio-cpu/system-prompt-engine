...
## Traceable prompt review, 22 September 2026

Added a responsive "Why this prompt" result view showing the original goal, role, boundaries, working approach, deliverable and acceptance criteria. Each item identifies whether it came from the explicit brief or an editorial default and explains its purpose. Open questions come only from returned user-supplied uncertainty entries; an empty list is not a completeness claim. Editing inputs invalidates the explanation together with the prompt. Imported artifacts require recompilation before a new explanation is available.

Browser validation confirmed compilation, the review view, default versus supplied-role labels, invalidation on edit, and a readable 390-pixel mobile layout. Desktop and mobile screenshots are retained locally in proofs/web04/04-prompt-review-desktop.png and 05-prompt-review-mobile.png. The Coding template now distinguishes review, explanation and implementation requests instead of always asking for code changes. Real-WASM regression covers all 13 categories and the explanation's source attribution. No comparative or independent 10/10 rating is claimed.
