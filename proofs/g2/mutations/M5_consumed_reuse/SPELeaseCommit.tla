...
CommitSuccess(p) ==
  ...
  /\ LET l == patchLease[p] IN
       ...
       /\ leaseStatus' = [leaseStatus EXCEPT ![l] = "ACTIVE"]  \* MUTANT M5: no consume
       /\ leaseBase' = [leaseBase EXCEPT ![l] = curVersion + 1] \* MUTANT M5: rebind for reuse
       /\ patchCommitted' = [patchCommitted EXCEPT ![p] = TRUE]
       /\ committedAgainst' = [committedAgainst EXCEPT ![p] = curVersion]
  /\ UNCHANGED <<
       leaseObl, leaseReqType,
...
\* Continuously enabled valid commit eventually resolves (L1)
